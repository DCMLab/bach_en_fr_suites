...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/bach_en_fr_suites.git
...
* `J.S. Bach – English and French Suites <https://github.com/DCMLab/bach_en_fr_suites/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the *Prélude* of the first English Suite, BWV 806, has the following files:

-  ``MS3/BWV806_01_Prelude.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/BWV806_01_Prelude.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/BWV806_01_Prelude.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/BWV806_01_Prelude.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/BWV806_01_Prelude.tsv')
   notes = ms3.load_tsv('notes/BWV806_01_Prelude.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/bach_en_fr_suites/issues>`__ and feel free to fork and submit pull requests.
